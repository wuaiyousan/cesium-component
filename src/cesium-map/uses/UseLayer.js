 ...
 * @LastEditTime: 2026-02-11 11:13:59
 * @Description: UseLayer
 */
import Cesium from "../utils/exportCesium.js";
import GCJ02UrlTemplateImageryProvider from "../utils/Transformation/GCJ02UrlTemplateImageryProvider.js";
export default function UseLayer() {
  ...
  function loadTerrainProvider(layerNode, toEarth = window.earthObj) {
    ...
    if (layerNode.type === "ArcGISTiledElevationTerrainProvider") {
      theViewer.terrainProvider = new Cesium.ArcGISTiledElevationTerrainProvider({
        url: layerNode.url,
      });
  ...
  // 加载TMS服务
  async function loadTmsLayer(layerNode, toEarth = window.earthObj) {
    let theLayer = findLayerByParam(layerNode, toEarth);
    ...
    if (theLayer) {
      ...
      let theViewer = toEarth;
      let { url, params } = layerNode;
      const tms = await Cesium.TileMapServiceImageryProvider.fromUrl(url, {
        fileExtension: params.fileExtension || "jpg",
        minimumLevel: params.minimumLevel || 4,
        maximumLevel: params.maximumLevel || 18,
        rectangle: new Cesium.Rectangle(
          Cesium.Math.toRadians(params.Rectangle[0] || -180.0),
          Cesium.Math.toRadians(params.Rectangle[1] || -85.0),
          Cesium.Math.toRadians(params.Rectangle[2] || 180.0),
          Cesium.Math.toRadians(params.Rectangle[3] || 85.0),
        ),
      });
      let imageryLayer = theViewer.imageryLayers.addImageryProvider(tms);
      imageryLayer.id = layerNode.id;
    }
  }

  // 加载XYZ服务
  function loadXYZLayer(layerNode, toEarth = window.earthObj) {
    let theLayer = findLayerByParam(layerNode, toEarth);
    ...
    if (theLayer) {
      ...
      let theViewer = toEarth;
      let { url, params } = layerNode;
      const xyzProvider = new GCJ02UrlTemplateImageryProvider({
        url:"http://220.170.90.49:8004/tilesData/google/{z}/{x}/{y}.png",
        // 限制层级范围
        minimumLevel: params.minimumLevel || 0,
        maximumLevel: params.maximumLevel || 18, // 根据服务实际情况调整
      });

      let imageryLayer = theViewer.imageryLayers.addImageryProvider(xyzProvider);
      imageryLayer.id = layerNode.id;
    }
  }
  // 隐藏图层
  ...
  function findLayerByParam(layerNode, toEarth = window.earthObj) {
    ...
    for (let i = 0, len = layerCollection.length, tmpLayer = null; i < len; i++) {
  ...
  // 格式化数据
  function formatList({ list, isAddValue = false }) {
    let out = [];
    //过滤geometry为空的数据
